  ...
  const calculateTotals = () => {
    ...
    if (order && order.orderLineItems) {
        order.orderLineItems.forEach(item => {
            ...
            const inputQty = inputQtyString !== '' ? parseInt(inputQtyString, 10) : 0;
            const unitPrice = parseFloat(item.unitPrice) || 0;
            const gst = parseFloat(item.gst) || 0;

            // Debugging logs
            // console.log('Item ID:', item.orderLineitemId);
            // console.log('Shipped Qty:', shippedQty);
            // console.log('Received Qty:', receivedQty);
            // console.log('Input Qty String:', inputQtyString);
            // console.log('Input Qty:', inputQty);
            // console.log('Checkbox State (isChecked):', isChecked);

            // Calculate qty based on checkbox state
            console.log("inputQty=====>inputQty=====>", inputQty);
            let qty;
            if (isChecked) {
                qty = inputQty > 0 ? receivedQty + inputQty : shippedQty;
            } else {
                qty = receivedQty;
            }

            // Calculate gross value based on receivedQty + inputQty and unitPrice
            const totalQtyForItem = receivedQty + inputQty; // Total quantity to use for gross calculation
            ...
            // Debugging logs
            console.log("Accumulated totalQty:", totalQty);
            console.log('Calculated Qty:', qty);
            console.log('Current totalGst:', totalGst);
            console.log('Current totalCost:', totalCost);
        });
    }

    // Debugging log before setting state
    console.log('Final totalQty:', totalQty);
    console.log('Final totalGst:', totalGst);
    ...
    // Debugging log after setting state
    console.log('Totals state updated:', { totalQty: Math.floor(totalQty), totalGst: totalGst.toFixed(2), totalCost: totalCost.toFixed(2) });
};
...
useEffect(() => {
  ...
  const handleQtyChange = (text, id) => {
    const updatedInputValues = { ...inputValues, [id]: text };
    setInputValues(updatedInputValues);
    calculateTotals(updatedInputValues);  // Recalculate totals with updated input values
  };
  ...
  const renderOrderLineItem = ({ item }) => {
    ...
    const qty = parseInt(inputQty)+receivedQty || 0;
    const grnGross = qty * unitPrice;
    const grossWithoutDecimals = grnGross.toFixed(2);
